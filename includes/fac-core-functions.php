...
function fac_update_fathom_event($event_id, $name) {
    ...
    $_site_id   = $fac4wp_options[ FAC_OPTION_SITE_ID ];
    $return = [];
    if(empty($_site_id) || empty($event_id) || empty($name)) return [];
...
function fac_fathom_api($url='') {
    ...
    $_site_id   = $fac4wp_options[ FAC_OPTION_SITE_ID ];
    if(empty($url) || empty($_site_id) || empty($_api_key)) return $return;
    ...
    if ( ! is_wp_error( $response ) ) {
        ...
        if($response_code !== 200) {
            if(!empty($result)) {
                ...
                if(isset($result['error']) && !empty($result['error'])) $error_msg = $result['error'];
            }
        }
        else {
